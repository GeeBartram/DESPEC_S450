...
echo "Run: $line"
echo "z1shift: $z1shift"
echo "z2shift: $z2shift"
 ...
 #GERMANIUM CALIBRATION
  if [ $RUN -lt 39 ]; then
   ...
   cp ../Configuration_Files/Germanium/Coefficients/run53_calib.txt ../Configuration_Files/Germanium/Germanium_Energy_Calibration.txt
   cp ../Configuration_Files/2D_Gates/ID_x4AoQ_OsSetting.txt ../Configuration_Files/2D_Gates/ID_x4AoQ.txt
   cp ../Configuration_Files/2D_Gates/ID_ZvsAoQ_os.txt ../Configuration_Files/2D_Gates/ID_ZvsAoQ.txt
  else [ $RUN -lt 77 ]
   cp ../Configuration_Files/Germanium/Coefficients/run75_calib.txt ../Configuration_Files/Germanium/Germanium_Energy_Calibration.txt
   cp ../Configuration_Files/2D_Gates/ID_x4AoQ_OsSetting.txt ../Configuration_Files/2D_Gates/ID_x4AoQ.txt
   cp ../Configuration_Files/2D_Gates/ID_ZvsAoQ_os.txt ../Configuration_Files/2D_Gates/ID_ZvsAoQ.txt
  fi
...
void setup_s450_011_2022()
{
 ...
 /*
  ...
  frs->rho0[1]   = 1.; //S2-S4
  frs->rho0[2]   = 1.; //S4-S8
  frs->dispersion[0]    = -6.490201; // UPDATED 19/01/23 TA2-S2
  frs->magnification[0] = 0.7681765; // ADDED 19/01/23 TA2-S2
  frs->dispersion[1]    =  7.786034; // UPDATED 19/01/23 S2-S4
  frs->magnification[1] =  1.202305; // UPDATED 19/01/23 S2-S4
  ...
  //S2
  frs->dist_focS2 = 2258.799; // UPDATED 19/01/23 IN MILLIMETRES
  ...
  frs->dist_focS4   = 2282.1; // UPDATED 19/01/23
  ...
  id->x_s2_select   = 1; //1=tpc,2=sc21,3=sc22 GEE
  ...
  id->max_aoq_plot = 2.8;
  id->min_z_plot   = 40;
  id->max_z_plot   = 90;' > ../Configuration_Files/FRS/FRS_Setups/setup_s450_011_2022.C


  if [ $RUN -lt 39 ]; then
  echo "// bfield (Tm) for new control system. (we put rho = 1)
  frs->bfield[0] = 13.2224; // 208Pb, E=1000 MeV/u Please do NOT comment-out old brho and add new brho. Please make a NEW setup file.
  frs->bfield[1] = 13.2224; // Please do NOT comment-out old brho and add new brho. Please make a NEW setup file.
  frs->bfield[2] = 9.5671;//203Ir // Please do NOT comment-out old brho and add new brho. Please make a NEW setup file. preparing for S450
  frs->bfield[3] = 9.5671;//203Ir // Please do NOT comment-out old brho and add new brho. Please make a NEW setup file. preparing for S450
  frs->bfield[4] = 4.8347; // D5 (to ESR) not used
  frs->bfield[5] = 4.8347; // D6 (to S8)" >> ../Configuration_Files/FRS/FRS_Setups/setup_s450_011_2022.C
  else
  echo "// bfield (Tm) for new control system. (we put rho = 1)
  ...
  echo "// TOF calibration SC21-SC81 (TAC)
...
//not related for S8
  id->offset_z   = $z1shift; //run75
  id->offset_z2  = $z2shift; //run75
  id->offset_z3  = 0.0;
...
//  sci->vftx_offset_228  =  -170.2; //ns // DUMMY needs calibration
  ...
  Float_t my_cID_dEToF_points[4][2] =
    {{    0.,    0.},
     {    0., 4000.},
     {40000., 4000.},
     {40000.,    0.}};" >> ../Configuration_Files/FRS/FRS_Setups/setup_s450_011_2022.C



  echo '//an->SetupPolyCond("cID_dEToF", 4, my_cID_dEToF_points);
...
}' >> ../Configuration_Files/FRS/FRS_Setups/setup_s450_011_2022.C
